...
Terrain.prototype.build = function(voronoi, options) {
    ...
    for (var i = 0; i < polys.length; i++) {
        ...
        THREE.ShapeUtils.triangulate(vertices).map((triangle) => {
            ...
            triangle[0].z = calculateHeight(triangle[0].x, triangle[0].y);
            triangle[1].z = calculateHeight(triangle[1].x, triangle[1].y);
            triangle[2].z = calculateHeight(triangle[2].x, triangle[2].y);

            var height = (triangle[0].z + triangle[1].z + triangle[2].z) / 3;            
            var color = calculateColor(height);

            var face = new THREE.Face3(index, index+1, index+2, normal, color);
            
            this.mapGeometry.vertices.push(triangle[0], triangle[1], triangle[2]);
            this.mapGeometry.faces.push(face);

            if (flat) {
                triangle[0].z = triangle[1].z = triangle[2].z = 0;
            }
        });
    }
};
...
Terrain.prototype.createMesh = function(options) {
    ...
    // MAP
    var mapMaterial = new THREE.MeshBasicMaterial( { side: THREE.DoubleSide, vertexColors: THREE.FaceColors } );
