...
class Terrain {
     ...
     * Build terrain.
     * @memberof Terrain
     * @param voronoi {voronoi} the voronoi diagram.
    ...
    build(voronoi, options) {
         ...
         * cell.faces - collection of faces
         * edge.slope - 
         */
        ...
        for (let i = 0; i < cells.length; i++) {
            ...
            THREE.ShapeUtils.triangulate(vertices).map((triangle) => {
                ...
                color = color || calculateColor(height, triangle[0].x, triangle[0].z);
     ...
     * @param options.borderColor {THREE.Color} the color for the cell border
     * @param options.borderWidth {number} cell border width
     * @param options.siteSize {number} the size of a site point
     */
    createMesh(options) {
        options = options || {};
        ...
        var mapMaterial = options.mapMaterial || new THREE.MeshBasicMaterial( { side: THREE.FrontSide, vertexColors: THREE.FaceColors } );
